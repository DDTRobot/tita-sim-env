...
{
    WbNodeRef physics_node;
    WbFieldRef physics_mass;
    double mass = 0;
    for(uint8_t id = 0; id < LINK_NUM; id++)
    {     
        char physics[100];
...
void initDevices()
{
    ...
    const WbNodeRef root_node = wb_supervisor_node_get_root();
    const WbFieldRef root_children_field = wb_supervisor_node_get_field(root_node, "children");
    WbNodeRef node = wb_supervisor_field_get_mf_node(root_children_field, 0);
    WbFieldRef field = wb_supervisor_node_get_field(node, "basicTimeStep");
    wb_supervisor_field_set_sf_float(field, TIME_STEP);

    WbDeviceTag motor[MOTOR_NUM]; 
    WbDeviceTag pos_sensor[MOTOR_NUM];
    for(uint8_t id = 0; id < MOTOR_NUM; id++)
    {
...
// privite
void calcRelative(const double *base, const double *target,  const double *rotation, double *relative)
{
